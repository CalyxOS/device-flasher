...
func getPlatformTools() error {
	platformToolsURLMap := map[[2]string]string{
		...
		[2]string{"darwin", "37.0.1"}: "https://release.calyxos.org/device-flasher/platform-tools/platform-tools_r37.0.1-darwin.zip",
		// https://dl.google.com/android/repository/platform-tools_r37.0.1-linux.zip
		[2]string{"linux", "37.0.1"}: "https://release.calyxos.org/device-flasher/platform-tools/platform-tools_r37.0.1-linux.zip",
		// https://web.archive.org/web/20260731070637/https://dl.google.com/android/repository/platform-tools-latest-windows.zip / all >= 35.0.1 URLs are broken
		[2]string{"windows", "37.0.1"}: "https://release.calyxos.org/device-flasher/platform-tools/platform-tools_r37.0.1-windows.zip",
	}
...
func flashDevices(devices map[string]string) {
	...
	for serialNumber, device := range devices {
		...
		go func(serialNumber, device string) {
			...
			if device == "FP4" || device == "FP5" || device == "FP6" || device == "otter" {
			...
			if len(downgrades) > 0 {
				errorln("The Anti-Rollback protection has stopped the bootloader relocking of your "+device+" "+serialNumber, false)
				errorln("CalyxOS has been fully installed. However, the flashed factory image has a lower Anti-Rollback index than the previous OS installed on this device:", false)
				for _, downgrade := range downgrades {
					warnln("  " + downgrade)
				}
				errorln("Locking the bootloader now will prevent the device from booting. It will stay unlocked.", false)
				errorln("Do not try to lock the bootloader manually when you see this warning.", false)
				errorln("We recommend that you download and flash a newer factory image to be able to relock the bootloader for maximum security.", false)
				errorln("Visit https://calyxos.org/motorola-relock for more information.", true)
				return
			} else {
