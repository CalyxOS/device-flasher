...
// no
func getLocked(serialNumber string, device string) bool {
...
func getUnlocked(serialNumber string, device string) bool {
	if device == "devon" || device == "hawao" || device == "rhode" {
...
func flashDevices(devices map[string]string) {
	...
	for serialNumber, device := range devices {
		...
		go func(serialNumber, device string) {
			...
			for i := 0; !getUnlocked(serialNumber, device); i++ {
				platformToolCommand = *fastboot
			...
			for i := 0; getLocked(serialNumber, device); i++ {
