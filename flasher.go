...
// no
func isLocked(serialNumber string, device string) bool {
...
func isUnlocked(serialNumber string, device string) bool {
	if device == "devon" || device == "hawao" || device == "rhode" {
...
func flashDevices(devices map[string]string) {
	...
	for serialNumber, device := range devices {
		...
		go func(serialNumber, device string) {
			...
			for i := 0; !isUnlocked(serialNumber, device); i++ {
				platformToolCommand = *fastboot
			...
			for i := 0; !isLocked(serialNumber, device); i++ {
